...
TEST(MetricsOverhead, AttachingMetricsDoesNotMeaningfullySlowPublish) {
    ...
    auto run = [&](bool with_metrics) {
        ...
        Broker broker(dir);
        auto topic = broker.create_topic("overhead", 1, 64 * 1024 * 1024, 0, FlushPolicy::Manual);
    ...
    std::vector<double> without_runs;
    std::vector<double> with_runs;

    constexpr int kBenchmarkRuns = 5;

    for (int i = 0; i < kBenchmarkRuns; ++i) {
        without_runs.push_back(run(false));
        with_runs.push_back(run(true));
    }

    std::sort(without_runs.begin(), without_runs.end());
    std::sort(with_runs.begin(), with_runs.end());

    double without = without_runs[kBenchmarkRuns / 2];
    double with = with_runs[kBenchmarkRuns / 2];
